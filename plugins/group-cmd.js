...
if (!isAdmins) return reply('You Are Not A Admin')	
  if (args[1] == 'second') {
                ...
                setTimeout(() => {
                    ...
                    const open = `*OPEN TIME* THE GROUP WAS OPENED BY CyberEagle TO APPROVED ADMIN\n NOW MEMBERS CAN SEND MESSAGES 🔓`
...
if (!isAdmins) return reply('You Are Not A Admin')	
                ...
                setTimeout(() => {
                    ...
                    const close = `*CLOSE TIME* GROUP CLOSED BY CyberEagle AT APPROVED ADMIN\nNOW ONLY ADMIN CAN SEND MESSAGES 🔐`
...
if (!isAdmins) return reply('You Are Not A Admin')
const G_MUTE = `*GROUP IS CLOSED BY CyberEagle BOT OWNER*`;
...
if (!isBotAdmins) return reply('You Not A Bot Owner')
if (!isAdmins) return reply('You Are Not A Admin')
const G_UNMUTE = `*GROUP IS OPEN BY CyberEagle BOT OWNER*`;
...
await conn.sendMessage(from, { react: { text: `✅`, key: mek.key }}) 
reply('*GROUP ADMIN PROMOTE BY CyberEagle BOT OWNER*')
} catch (e) {
reply('*ERROR*')
console.log(e)
}
...
await conn.sendMessage(from, { react: { text: `✅`, key: mek.key }})
reply('*GROUP ADMIN DEMOTE BY CyberEagle BOT OWNER*') 
} catch (e) {
reply('*ERROR*')
console.log(e)
}
...
if (!isOwner && !isAdmins) return reply('You must be an admin to use this command.');
try{
