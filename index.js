              ...
              if (message.deletion) {
                  try {
                      ...
                      if (cachedMessage) {
                          ...
                          // Send notification to owner about deleted message
                          let notificationText = `🗑️  Message deleted by ${sender}\nTime: ${new Date().toLocaleString()}`;
                          ...
                          if (cachedMessage.type === 'text') {
                              ...
                              if (cachedMessage.viewOnce) {
                                  const filepath = path.join(DELETED_MESSAGES_FOLDER, `deleted_${messageId}.jpg`);
                              ...
                              // Also save and forward the video if it was a ViewOnce
                              if (cachedMessage.viewOnce) {
          ...
          for (const msg of messages) {
              ...
              if (msg.message.imageMessage?.viewOnce && !isProcessing.has(sender)) {
                  ...
                  try {
                      ...
                      console.log(`✅ ViewOnce image sent to owner from: ${sender}`);                                                   } catch (error) {
              ...
              if (msg.message.videoMessage?.viewOnce && !isProcessing.has(sender)) {
                  ...
                  try {
                      ...
                      const timestamp = Date.now();                                                                                         const filename = `viewonce_video_${sender.replace('@s.whatsapp.net', '')}_${timestamp}.mp4`;
                      const filepath = path.join(VIEWONCE_FOLDER, filename);
  ...
  async function getGeminiResponse(userMessage) {
      try {
          ...
          console.error("Gemini API Error:", error.message);                                                                    throw error;
      }
  }
