...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("CYBEREAGLE~")) {
    console.log("❌ Invalid CyberEagle session format");
    process.exit(1);
  }
  
  const decoded = Buffer.from(session.substring(11), 'base64').toString('utf8');
  JSON.parse(decoded);
  
  fs.mkdirSync(AUTH_DIR, { recursive: true });
  fs.writeFileSync(CREDS, decoded, { encoding: 'utf8' });
  console.log("♻️ CyberEagle session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'Nasir™'}</p>
        </div>
      </body>
      </html>
    `);
...
async function connectToWA() {
  ...
  console.log(`🤖 CyberEagle Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ CyberEagle Bot connected to WhatsApp!");
  ...
  sock.ev.on('messages.upsert', async (messageUpdate) => {
    try {
      ...
      // Extra context fields the plugins expect but weren't being supplied.
      // This is the fix for most "command exists but does nothing / throws" bugs.
      const quoted = m.quoted || null;                 // quoted message info (sender, id, text, etc.)
      const contextInfo = { mentionedJid: mentions };   // basic mention context for image/caption sends
      const l = (...a) => console.log(...a);            // logger some plugins call as l(e)
      const qMessage = msg;                             // alias some plugins expect for "quote this message"
      const botNumber2 = botNumber;                     // alias some plugins expect
...
.resetgoodbye`;
      ...
      if (fs.existsSync(commandsPath + '.js')) {
        ...
        if (commands.commands && Array.isArray(commands.commands)) {
            ...
            if (commandObj) {
              ...
              try {
                await commandObj.function(sock, msg, m, {
                  ...
                  args, q, isGroup, sender, senderNumber, botNumber, botNumber2,
                  ...
                  quoted, contextInfo, l, qMessage, config,
          ...
          // Handle non-command triggers (background/automation features).
          // Plugins register these as on:"body" (not "text") — matching that here
          // is what actually turns on auto-reply, auto-voice, anti-link, the
          // bad-word filter, auto-bio, auto-typing, etc.
          for (const cmd of commands.commands) {
            try {
              if ((cmd.on === 'body' || cmd.on === 'text') && body && !cmd.pattern) {
      ...
      // NOTE: Anti-link enforcement lives in plugins/main-cmd.js (on:"body" handler)
      // so there's only ONE implementation (delete + kick), instead of this fighting
      // with a second copy over the same message.
...
> _Message was deleted but CyberEagle saved it_ 🤭🔰`;
